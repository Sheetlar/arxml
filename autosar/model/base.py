...
class FloatLimit(float, Limit):
    pass


class IntLimit(int, Limit):
    pass
...
def create_admin_data(data: dict[str, ...]):
    admin_data = AdminData()
    sdg_gid: str | None = data.get('SDG_GID', None)
    if 'SDG' in data:
...
class SwDataDefPropsVariants(Variants[SwDataDefProps]):
    pass
...
class SwPointerTargetProps:
    ...
    def __init__(self, target_category: str | None = None, variants: SwDataDefProps | Iterable | None = None):
        self.target_category = target_category
        if variants is None:
            self.variants = []
        elif isinstance(variants, SwDataDefProps):
...
class AbstractValueSpecification(ArObject):
    def __init__(
            self,
...
class NumericalValueSpecification(AbstractValueSpecification):
    def __init__(
            self,
...
class TextValueSpecification(AbstractValueSpecification):
    def __init__(
            self,
            value: str | None = None,
            *args, **kwargs,
...
ValueSpecification = TypeVar('ValueSpecification', bound=AbstractValueSpecification)


@dataclass
class DataFilter(ArObject):
    data_filter_type: str | None = None
    mask: int | None = None
    max: int | None = None
    min: int | None = None
