...
from autosar.model.behavior import InternalBehavior, SwcInternalBehavior
from autosar.model.builder import ValueBuilder
from autosar.model.element import Element, ParameterDataPrototype, SoftwareAddressMethod
from autosar.model.mode import ModeGroup, ModeDeclaration, ModeDeclarationGroup
...
class Package(ArObject):
    ...
    def create_application_record_data_type(
        ...
        if sw_calibration_access is not None:
            variant_props = SwDataDefProps(sw_calibration_access=sw_calibration_access)
        else:
            variant_props = None
    ...
    def create_implementation_data_type_ptr(
        ...
        target_props = SwPointerTargetProps(
            target_category,
            SwDataDefProps(base_type_ref=base_type_ref, sw_impl_policy=sw_impl_policy),
        )
        variant_props = SwDataDefProps(sw_pointer_target_props=target_props)
    ...
    def create_implementation_array_data_type(
        ...
        outer_props = SwDataDefProps(sw_calibration_access=sw_calibration_access)
        new_type.variant_props = [outer_props]
        inner_props = SwDataDefProps(implementation_type_ref=implementation_type_ref)
    ...
    def create_implementation_record_data_type(
        ...
        for element in elements:
            ...
            if isinstance(elem_type, ImplementationDataType):
                element_props = SwDataDefProps(implementation_type_ref=elem_type.ref)
            elif isinstance(elem_type, SwBaseType):
                element_props = SwDataDefProps(base_type_ref=elem_type.ref)
            else:
